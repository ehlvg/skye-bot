...
function toolSummary(toolCalls: unknown): string | null {
  ...
  return toolCalls
    .map((call) => {
      ...
      const tool = call as { name?: unknown; isConnector?: unknown; args?: unknown };
      const keys =
        tool.args && typeof tool.args === "object" ? Object.keys(tool.args as object) : [];
      return `${tool.isConnector ? "Connector" : "Tool"}: ${String(tool.name ?? "unknown")}${keys.length ? ` (${keys.join(", ")})` : ""}`;
    })
    .join(" · ");
}
    ...
    <div className="fade-in">
      ...
      {monitoring && (
        <Section>
          ...
          <Footnote>
            Live process status. Only bot administrators can see the audit timeline below.
          </Footnote>
        </Section>
      )}
      {events.length > 0 && (
                  ...
                  return (
                    <Fragment key={key}>
                      ...
                      >
                        ...
                        <td>
                          {event.status && (
                            <span className={`audit-status is-${event.status}`}>
                              {event.status}
                            </span>
                          )}
                        </td>
                      ...
                      {isOpen && (
                        <tr className="audit-expanded-row">
                          <td colSpan={4}>
                            ...
                            {details && (
                              <pre
                                className={event.error ? "audit-detail is-error" : "audit-detail"}
                              >
                                {details}
                              </pre>
                            )}
                            {!event.inputText && !event.outputText && !details && !tools && (
                              <div className="audit-empty">
                                No detailed content was captured for this older event.
                              </div>
                            )}
                          </td>
                        </tr>
